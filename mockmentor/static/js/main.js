...
// ── Mobile dark toggle (in navbar toggler row) ────────────────
const darkToggleMob = document.getElementById('darkToggleMob');
const darkIconMob   = document.getElementById('darkIconMob');

function syncMobIcon(isDark) {
    if (!darkIconMob) return;
    darkIconMob.className = isDark ? 'bi bi-sun-fill' : 'bi bi-moon-stars';
}

// Sync mobile icon on load
const _initDark = html.getAttribute('data-theme') === 'dark';
syncMobIcon(_initDark);

if (darkToggleMob) {
    darkToggleMob.addEventListener('click', () => {
        const isDark = html.getAttribute('data-theme') === 'dark';
        applyTheme(!isDark);
        syncMobIcon(!isDark);
    });
...
// Keep both dark icons in sync when desktop toggle is clicked
const _origApply = applyTheme;
// Patch applyTheme to also sync mobile icon
const _patchedApply = function(dark) {
    _origApply(dark);
    syncMobIcon(dark);
};
// Override for subsequent calls
window.applyTheme = _patchedApply;
if (darkBtn) {
    darkBtn.removeEventListener('click', darkBtn._handler);
    darkBtn._handler = () => {
        const isDark = html.getAttribute('data-theme') === 'dark';
        _patchedApply(!isDark);
    };
    darkBtn.addEventListener('click', darkBtn._handler);
}

// ── Bottom nav – highlight active link ───────────────────────
document.addEventListener('DOMContentLoaded', () => {
    const path    = window.location.pathname;
    const bnLinks = document.querySelectorAll('.mm-bottom-nav a');

    bnLinks.forEach(link => {
        const href = link.getAttribute('href');
        if (!href) return;
        // Mark active: exact match or prefix match (exclude '/')
        const isActive = href === path || (href.length > 1 && path.startsWith(href));
        if (isActive) link.classList.add('active');
    });

    // ── Navbar collapse: close on nav-link click (mobile) ────
    ...
    // ── Fix chart.js responsiveness on resize/orientation change
    ...
    document.querySelectorAll('.btn, .quick-action-card, .mm-bottom-nav a').forEach(el => {
        el.addEventListener('touchend', e => e.preventDefault(), { passive: false });
    });

    // ── Swipe to close navbar on mobile ──────────────────────
    let _touchStartY = 0;
    ...
    document.addEventListener('touchend', e => {
        ...
        if (dy < -30 && collapse && collapse.classList.contains('show')) {
